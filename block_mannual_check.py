...
# ------------------ USER URL INPUT (Plan C) ------------------
st.sidebar.header("🌐 Load input from URL")

raster_url = st.sidebar.text_input(
    "Raster URL (.tif, can be Dropbox / S3 / HTTP)",
    value=""
)
vector_url = st.sidebar.text_input(
    "Vector URL (.geojson)",
    value=""
)
ocr_url = st.sidebar.text_input(
    "OCR CSV URL (.csv)",
    value=""
)

output_subdir = st.sidebar.text_input(
    "Output folder name on server (relative, optional)",
    value="outputs"
)

if not (raster_url and vector_url and ocr_url):
    st.warning("👈 Please provide Raster / Vector / OCR URLs in the sidebar before continuing.")
    st.stop()

raster_url_norm = normalize_dropbox_url(raster_url)
vector_url_norm = normalize_dropbox_url(vector_url)
ocr_url_norm    = normalize_dropbox_url(ocr_url)

# Raster
src = rasterio.open(raster_url_norm)
# Vector
gdf0 = gpd.read_file(vector_url_norm)
# OCR CSV
ocr_df = pd.read_csv(ocr_url_norm, dtype=str, low_memory=False).fillna("")

OUTPUT_DIR = os.path.join(os.getcwd(), output_subdir or "outputs")
os.makedirs(OUTPUT_DIR, exist_ok=True)

ocr_name = Path(urlparse(ocr_url_norm).path).name or "ocr.csv"
WORK_CSV = os.path.join(
    OUTPUT_DIR,
    Path(ocr_name).stem + "__ui_work.csv"
)
# ---------------------------------------------------------
